...
def main():
    ...
    st.markdown("*Powered by LangGraph, spraply, and LiteLLM*")
    ...
    env_spraply_key = os.getenv("SPRAPLY_API_KEY")
    ...
    if env_spraply_key:
        st.sidebar.markdown('<small style="color: green;">✅ spraply API Key loaded from .env</small>', unsafe_allow_html=True)
    else:
        st.sidebar.markdown('<small style="color: orange;">⚠️ spraply API Key not found in .env</small>', unsafe_allow_html=True)
    ...
    if not env_spraply_key:
        spraply_key = st.sidebar.text_input(
            "spraply API Key", 
            type="password",
            help="Enter your spraply API key"
        )
    ...
    if spraply_key:
        os.environ["SPRAPLY_API_KEY"] = spraply_key
    if openai_key:
    ...
    has_spraply = bool(env_spraply_key or spraply_key)
    ...
    if prompt := st.chat_input("What would you like to search for?", disabled=st.session_state.search_running):
        ...
        if not has_spraply or not has_openai:
            with st.chat_message("assistant"):
                missing_keys = []
                if not has_spraply:
                    missing_keys.append("spraply")
                if not has_openai:
