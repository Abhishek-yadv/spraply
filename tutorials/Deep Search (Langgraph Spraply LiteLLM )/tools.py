...
Handles spraply integration, BM25 filtering, and LLM interactions.
"""
...
# Initialize spraply client
spraply_client = None

def init_spraply_client():
    """Initialize spraply client with API key from environment."""
    global spraply_client
    api_key = os.getenv("SPRAPLY_API_KEY")
    
    if not api_key:
        raise ValueError("SPRAPLY_API_KEY not found in environment variables")
    
    # Initialize with base_url as shown in working example
    spraply_client = SpraplyAPIClient(
        api_key=api_key, 
        base_url='https://app.spraply.dev'
    )
    log_event("spraply client initialized")
...
def spraply_search(
    ...
    """
    Search the web using spraply API.
    ...
    try:
        ...
        # Build search_options according to spraply API spec
        search_options = {
            "search_type": "web",
            "depth": depth
        }
        ...
        if time_range and time_range != "any":
            # Map common time ranges to spraply expected values
            time_range_map = {
                "hour": "hour",
                "day": "day", 
                "week": "week",
        ...
        try:
            ...
            result = spraply_client.create_search_request(
                query=query,
        ...
        if isinstance(result, dict):
            # spraply API returns results under 'result' key, not 'results'
        ...
        log_event(f"Error in spraply_search: {str(e)}", "error")
        
        # Log more detailed error information for debugging
        if hasattr(e, 'response'):
            try:
                error_details = e.response.text if hasattr(e.response, 'text') else str(e.response)
                log_event(f"spraply API error details: {error_details}", "error")
            except:
                log_event(f"Could not extract error details from response", "error")
        
        return []

def spraply_scrape(url: str, page_options: Optional[Dict] = None) -> Dict[str, Any]:
    """
    Scrape a single URL using spraply.
    ...
    try:
        ...
        result_object = spraply_client.scrape_url(
            url=url,
