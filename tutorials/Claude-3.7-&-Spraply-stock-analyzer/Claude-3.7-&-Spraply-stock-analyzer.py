#!/usr/bin/env python3
"""
Enhanced Stock Analyzer using spraply and Claude 3.7

This script provides comprehensive stock analysis by:
1. Searching for stock information using spraply
...
class StockAnalyzer:
    """
    A class to analyze stocks using spraply and Claude 3.7.
    """
    
    def __init__(self):
        """Initialize the StockAnalyzer with required API clients."""
        self.spraply_client = None
    ...
    def initialize_clients(self) -> None:
        ...
        # Retrieve API keys
        api_keys = {
            'SPRAPLY_API_KEY': os.getenv("SPRAPLY_API_KEY"),
        ...
        # Initialize clients
        self.spraply_client = SpraplyAPIClient(api_key=api_keys['SPRAPLY_API_KEY'])
    ...
    def find_relevant_stock_pages(self, stock_symbol: str, base_url: str) -> Optional[List[str]]:
        """
        Find relevant stock pages using spraply Web Search.
        ...
        """
        logger.info(f"Searching spraply for stock: {stock_symbol} on {base_url}")
        
        try:
            search = self.spraply_client.create_search_request(
            ...
            logger.error(f"Error finding stock pages with spraply: {str(e)}")
            return None
    
    def scrape_stock_data(self, urls: List[str]) -> List[Dict[str, str]]:
        """
        Scrape content from the given URLs using spraply.
        ...
        for page_url in urls[:5]:  # Limit to top 5 pages
            try:
                ...
                scrape_result = self.spraply_client.scrape_url(
                    url=page_url,
                    page_options={
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Enhanced Stock Analyzer using spraply and Claude 3.7')
