...
try
    from spraply import SpraplyAPIClient
except ImportError:
    from spraply import SpraplyAPIClient
...
@dataclass
class Spraplyer:
    """Client for spraply API with sitemap, search and scraping capabilities."""

    def __init__(self, api_key: str = None, debug_mode: bool = False):
        if api_key is None:
            api_key = os.getenv("SPRAPLY_API_KEY")
        if not api_key:
            raise ValueError("spraply API key is required")
        ...
        LOGGER.info("Initialized Spraplyer")

    # ───────────────────────── public façade ────────────────────────── #
    def sitemap_full(self, url: str) -> List[str]:
        """Return a complete sitemap of URLs within the domain."""
        request = SpraplyAPIClient(self.api_key).create_sitemap_request(
            url=url,
            options={
        ...
        # Wait for completion
        for event in SpraplyAPIClient(self.api_key).monitor_sitemap_request(req_id):
            if event["type"] == "state":
        ...
        raw: Any = SpraplyAPIClient(self.api_key).get_sitemap_results(req_id, output_format="json")

        # spraply may yield a JSON string, list[str], or list[dict]
    ...
    def search(self, query: str, search_options: Dict[str, Any], result_limit: int) -> List[Dict[str, Any]]:
        """Search for a query and return the results."""
        # Remove None values from search_options
        search_options = {k: v for k, v in search_options.items() if v is not None}
        search_options["result_limit"] = result_limit
        
        try:
            search_request = SpraplyAPIClient(self.api_key).create_search_request(
                query=query,
            ...
            search_id = getattr(search_request, "id", None) or search_request.get("id", None) or search_request.get("request_id", None) or search_request.get("uuid", None)
            if not search_id:
                LOGGER.error("Failed to create search request: No ID found in response. Response: %s", search_request)
                return []
            
            LOGGER.info("Search request created with ID: %s", search_id)
            search_results = getattr(search_request, "results", []) or search_request.get("results", []) or search_request.get("result", [])
            if not search_results:
                LOGGER.warning("No results found in search request response. Status: %s", getattr(search_request, "status", "Unknown") or search_request.get("status", "Unknown"))
                return []
            return search_results
        except Exception as e:
            LOGGER.error("Error creating search request: %s", str(e))
            return []

    def monitor_search(self, search_id: str, download: bool = True) -> Iterator[Dict[str, Any]]:
        """Monitor a search request and yield events."""
        for event in SpraplyAPIClient(self.api_key).monitor_search_request(search_id, download=download):
            yield event

    def get_search_details(self, search_id: str, download: bool = True) -> Dict[str, Any]:
        """Get search request details."""
        return SpraplyAPIClient(self.api_key).get_search_request(search_id, download=download)
        
    def scrape(self, url: str) -> Dict[str, Any]:
        """Download *url* and return spraply's result object."""
        result = SpraplyAPIClient(self.api_key).scrape_url(
            url,
            page_options={
        ...
        if result is None:
            LOGGER.warning(f"Scrape returned None for URL: {url}")
            return None
        return result.get("result", {}).get("markdown", None)
