## 🚀 Objective-Driven Web Crawling with Spraply, LiteLLM, and Rank-BM25
...
├── objective_crawler/
│   ├── __init__.py
│   ├── clients.py        # API client wrappers for Spraply and LLM interactions
...
1. **Install Dependencies**:
   ...
   ```
   This will install essential libraries like `litellm`, `rank-bm25`, `Spraply-sdk`, and `python-dotenv`.

2. **Set Up Environment Variables**:
   - Copy the `.env.example` file to a new file named `.env`.
   - Open the `.env` file and fill in your API keys for Spraply and the LLM providers you intend to use (e.g., OpenAI, Anthropic, DeepSeek). These keys are crucial for accessing the respective services.
   ```bash
   cp .env.example .env
   ```
...
3. **Verify Setup**:
   ...
   ```bash
   python -c "import litellm, rank_bm25, Spraply_sdk; print('Setup successful!')"
   ```
   If no errors appear, your environment is ready.

### 📖 Detailed Overview of Core Technologies

#### 🌊 Spraply: Simplified Web search and Scraping

Spraply provides robust web search and scraping capabilities, from extracting content from searching the entire web,to scraping single pages to mapping entire websites through deep, structured crawls. Its real-time data retrieval and ease of integration make it a cornerstone for AI-driven content ingestion in ObjectiveCrawler. With Spraply, users can configure search depth, filter results by language or country, and focus on main content for efficient processing.
...
- **`Spraplyer`**: Handles interactions with the Spraply API, performing tasks like sitemap extraction, search operations, and content scraping with configurable options for depth and focus.
...
3. **📡 Data Source Acquisition (Spraply)**  
   If a URL is provided, ObjectiveCrawler search the web using Spraply to get candidate pages.  It performs a search via the Spraply API with configurable depth (basic, advanced, ultimate), language, and country settings to gather candidate URLs. This step ensures a broad pool of potential sources.
...
6. **📥 Content Scraping (Spraply)**  
   Content from the top-ranked URLs is scraped using Spraply, focusing on main content in markdown format for efficient processing. Options to include links or exclude irrelevant HTML elements (like ads or navigation bars) enhance the quality of data extracted.
...
- **⚡ Efficiency**: Handles vast and complex website data quickly by leveraging Spraply's rapid scraping and BM25's fast keyword ranking.
...
python main.py https://spraply.dev "Find rates or pricing" --top-k 10 --strategies 5 --search-llm gpt-4o-mini --ranking-llm anthropic/claude-3-haiku-20240307 --content-analysis-llm deepseek/deepseek-chat --reasoning-llm gpt-4.1 --debug
...
- **API Key Issues**: Ensure your Spraply and LLM provider API keys are correctly set in the `.env` file. Invalid or expired keys will cause authentication errors.
...
- **Network Issues**: Ensure a stable internet connection as Spraply requires online access for scraping and API calls.
