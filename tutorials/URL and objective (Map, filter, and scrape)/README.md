## 🚀 Web Data Extraction and Analysis with Spraply (Map and Scrape functions), LiteLLM, and Rank-BM25
...
This project implements a goal-oriented web crawler that uses Spraply for site mapping and Large Language Models (LLMs) for content analysis to fulfill a user-defined objective on a target website.

##### you give a URL (even a base URL of a website) and an objective (what you want to find on the website), then the tool will use Spraply to map the whole website, find the relevant pages using two stage filtering (1st using BM25, then using LLMs to filter out the best URLs), scrape the content of the nominated  pages, and then use LLMs to analyze the content of the nominated pages to find the information you want.
...
👤 User Input (Website URL + Objective)
        ⬇️
🕸️ Fetch Complete Sitemap with Spraply map function
        ⬇️
...
├── objective_crawler/
│   ├── __init__.py
│   ├── clients.py        # API client wrappers (Spraply, LLM)
...
2.  **Set up environment variables**:
    ...
    - Open the `.env` file and fill in your API keys for Spraply and the LLM providers you intend to use.
    ```bash
    cp .env.example .env
    ```
...
#### 🌊 Spraply: Simplified Web Scraping

Spraply delivers streamlined web crawling, from extracting single pages to managing deep, structured crawls. Its ease of use and powerful, real-time capabilities make it an essential tool for AI-driven content ingestion.
...
* **`Spraplyer`**: Manages interactions with the Spraply API, including scraping and sitemap extraction.
...
2. **📡 Complete Sitemap Extraction (Spraply)**
...
python main.py "https://spraply.dev/" "rates or pricing" --top-k 10 --strategies 5 --model anthropic/claude-sonnet-4-20250514  --debug
# Find pricing information with debug output enabled
python main.py https://example.com "Find pricing information" --debug
python main.py "https://spraply.dev/" "rates or pricing" --top-k 4 --strategies 5 --model deepseek/deepseek-chat --debug
```
