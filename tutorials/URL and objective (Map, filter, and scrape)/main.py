#!/usr/bin/env python3
"""gpt41_professional.py – Goal‑oriented site crawler powered by spraply & GPT‑4.1
...
from objective_crawler.clients import LLMClient, Spraplyer
...
def main() -> None:  # noqa: D401
    ...
    if not wc_key:
        _early_exit("SPRAPLY_API_KEY must be set in environment variables.")
    # if not wc_key or not oa_key:
    #     _early_exit("SPRAPLY_API_KEY and the API key for the model must be set in environment variables.")
    ...
    crawler = ObjectiveCrawler(
        wc=Spraplyer(wc_key, debug_mode=args.debug),
