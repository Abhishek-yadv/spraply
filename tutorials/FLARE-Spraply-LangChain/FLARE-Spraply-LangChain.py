...
#!/usr/bin/env python3
"""
spraply_flare_rag.py
====================================================

End‑to‑end RAG playground that combines

  • spraply  – crawl, scrape, and search
...
2. Runs either spraply Search (default) or Tavily to get fresh links.
3. Scrapes each link with spraply and wraps results in LangChain
   `Document`s.
...
----------------------------------------------------
    ...
    pip install python-dotenv requests \
                ...
                langchain tavily-python spraply-py
"""
...
logger = logging.getLogger("spraply_flare_rag")

###############################################################################
# Search helpers (spraply + Tavily)
###############################################################################
...
def spraply_search_tool(
    ...
    *,
    ...
    """Return top‐N URLs from spraply Search."""
    client = SpraplyAPIClient(api_key)
    try:
        ...
        logger.debug("spraply URLs: %s", urls)
        return urls[: max_results]
    except Exception as exc:  # noqa: BLE001
        logger.warning("spraply search failed: %s", exc)
        return []
...
###############################################################################
class SpraplyRetriever(BaseRetriever, BaseModel):
    """Custom LangChain retriever using spraply scrape."""

    client: SpraplyAPIClient
    tavily_api_key: Optional[str] = None
    SPRAPLY_API_KEY: Optional[str] = None
    search_tool: Callable[[str, str, int], List[str]] = Field(
        default_factory=lambda: spraply_search_tool
    )
    ...
    def _get_relevant_documents(
        ...
        **kwargs: Any,
        ...
        api_key = (
            ...
            else (self.SPRAPLY_API_KEY or getattr(self.client, "api_key", None))
        )
...
def run_query(
    ...
    wc_key = os.getenv("SPRAPLY_API_KEY")
    tavily_key = os.getenv("TAVILY_API_KEY")

    if not openai_key or not wc_key:
        raise RuntimeError("OPENAI_API_KEY and SPRAPLY_API_KEY must be set.")

    # 2‑ Build retriever
    retriever = SpraplyRetriever(
        client=SpraplyAPIClient(api_key=wc_key),
        SPRAPLY_API_KEY=wc_key,
        tavily_api_key=tavily_key,
        search_tool=tavily_search_tool if use_tavily else spraply_search_tool,
    )
...
def parse_args() -> argparse.Namespace:  # noqa: D401
    parser = argparse.ArgumentParser(description="spraply × FLARE RAG playground")
    parser.add_argument(
        "-q",
        "--query",
        required=True,
        help='User question in quotes, e.g. --query "What is spraply?"',
    )
    parser.add_argument(
        "--model",
        default="gpt-4o",
    ...
    parser.add_argument(
        "--use-tavily",
        action="store_true",
        help="Use Tavily instead of spraply Search (requires TAVILY_API_KEY)",
    )
    parser.add_argument(
        "-v",
        "--verbose",
...
# python FLARE-spraply-LangChain.py -q "Explain what is spraply tool and how I can improve the LLM performance?"