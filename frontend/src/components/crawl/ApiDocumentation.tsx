...
export const ApiDocumentation: React.FC<ApiDocumentationProps> = ({ request, isBatch }) => {
  ...
  const generatePythonCode = useCallback((request: CrawlRequest | null) => {
    ...
    if (isBatch) {
      ...
      return `from Spraply import SpraplyAPIClient

# Initialize the client
client = SpraplyAPIClient(api_key='${apiKey}', base_url='${baseUrl}')
...
crawl_request = client.create_crawl_request(
  ...
  const generateNodeCode = useCallback((request: CrawlRequest | null) => {
    ...
    if (isBatch) {
      const urls = request?.urls || ['https://example.com', 'https://example.org'];
      return `import { SpraplyAPIClient } from '@Spraply/nodejs';

// Initialize the client with your API key
const client = new SpraplyAPIClient('${apiKey}', '${baseUrl}');
...
console.log(crawlRequest);`;
  ...
  const generateGoCode = useCallback((request: CrawlRequest | null) => {
    ...
    if (isBatch) {
      const urls = request?.urls || ['https://example.com', 'https://example.org'];
      return `import "github.com/Abhishek-yadv/Spraply-go"

client := Spraply.NewClient("${apiKey}", "${baseUrl}")  // Empty string uses default base URL

ctx := context.Background()
input := Spraply.CreateBatchCrawlRequestInput{
    URLs: ${toGoMap(urls, 4, 3)},
    Options: Spraply.CrawlOptions{
...
ctx := context.Background()
input := Spraply.CreateCrawlRequestInput{
    URL: "${url}",
    Options: Spraply.CrawlOptions{
        SpiderOptions: ${toGoMap(spiderOptions, 4, 3)},
        PageOptions: ${toGoMap(pageOptions, 4, 3)},
                ...
                <div className="bg-[#1E1E1E] rounded-lg overflow-hidden">
                  ...
                  {tab.name === 'cURL' && (
                    <DocumentItem
                      ...
                      documentUrl="https://docs.spraply.dev/api/documentation/"
                  ...
                  {tab.name === 'Python' && (
                    <DocumentItem
                      ...
                      documentUrl="https://docs.spraply.dev/clients/python"
                      installCommand="pip install Spraply-py"
                      language="python"
                    />
                  )}
                  ...
                  {tab.name === 'Node.js' && (
                    <DocumentItem
                      ...
                      documentUrl="https://docs.spraply.dev/clients/nodejs"
                      installCommand="npm install @Spraply/nodejs"
                      language="javascript"
                    />
                  )}
                  ...
                  {tab.name === 'Go' && (
                    <DocumentItem
                      ...
                      documentUrl="https://github.com/Abhishek-yadv/Spraply-go"
                      installCommand="go get github.com/Abhishek-yadv/Spraply-go"
                      language="go"
                    />
                  )}
