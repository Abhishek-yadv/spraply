...
export const MCPServerAccess: React.FC = () => {
  ...
  const getMCPConfig = () => {
    ...
    switch (selectedApp) {
      ...
      case 'claude-code':
        return `claude mcp add Spraply -e Spraply_API_KEY=${apiKey} -- npx -y @Spraply/mcp --base-url ${baseUrl}`;
      default:
        return JSON.stringify({
          "mcpServers": {
            "Spraply": {
  ...
  const getMCPInstallLink = useCallback(() => {
    switch (selectedApp) {
      case 'cursor':
        return 'https://cursor.com/en/install-mcp?name=Spraply&config=' + btoa(JSON.stringify({
          "command": "npx",
          "args": [
            "@Spraply/mcp",
            "--base-url",
          ...
          "env": {
            "Spraply_API_KEY": selectedApiKey,
          }
        }));

      case 'vscode':
        return `https://insiders.vscode.dev/redirect/mcp/install?name=Spraply&config={"command":"npx","args":["-y","@Spraply/mcp", "--base-url", "${getBaseUrl()}"],"env":{"Spraply_API_KEY":"${selectedApiKey}"}}`;
      default:
        return null;
    }
                ...
                {apiKeys.length === 0 ? (
                  <div className="text-center py-12 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg">
                    ...
                    <p className="text-gray-500 dark:text-gray-400 mb-4 max-w-sm mx-auto">
                      You need to create an API key to access the Spraply API and use MCP integration
                    </p>
                    <a
