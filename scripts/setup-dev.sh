...
# Spraply Development Setup Script
...
echo -e "${COLOR_BLUE}🚀 Spraply Development Setup${COLOR_RESET}"
echo ""
...
# Setup backend
echo -e "${COLOR_BLUE}Setting up FastAPI backend...${COLOR_RESET}"
...
echo -e "${COLOR_GREEN}✓${COLOR_RESET} FastAPI backend setup complete!"
echo ""
...
echo -e "   ${COLOR_YELLOW}cd backend && alembic upgrade head${COLOR_RESET}"
echo ""
echo -e "3. Start the backend development server:"
echo -e "   ${COLOR_YELLOW}cd backend && uvicorn app.main:app --reload --host 0.0.0.0 --port 8000${COLOR_RESET}"
echo ""
echo -e "4. Start the frontend development server (in another terminal):"
