...
Spraply uses a single `.env` file in the `docker` directory for all configuration settings. This file contains all the necessary environment variables for the application, database, frontend, and other services.
...
|----------|---------------|-------------|
| `VERSION` | `v0.12.1`     | Application version. Always check [GitHub Releases](https://github.com/Abhishek-yadv/Spraply/releases) for the latest version |
...
| Variable | Default Value | Description |
|----------|---------------|-------------|
| `SECRET_KEY` | `change-me` | **Secret key for the FastAPI application. Generate a new one using `openssl rand -base64 32`. MUST be changed in production!** |
...
| Variable | Default Value | Description |
|----------|---------------|-------------|
| `REDIS_HOST_PORT` | `6379` | Redis host port mapping |
...
| Variable | Default Value | Description |
|----------|---------------|-------------|
| `EMAIL_BACKEND` | `smtp` | Email backend type |
...
| Variable | Default Value | Description |
|----------|---------------|-------------|
| `SCRAPY_USER_AGENT` | `Spraply/0.5.0 (+https://github.com/Abhishek-yadv/Spraply)` | User agent for web scraping |
...
```bash
# Core Settings
VERSION=v0.12.1  # Check https://github.com/Abhishek-yadv/Spraply/releases for latest version
NGINX_PORT=80
