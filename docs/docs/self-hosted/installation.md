...
5. **Create Initial Admin User**
   ```bash
   curl -X POST http://localhost/api/v1/user/install/ \
     -H "Content-Type: application/json" \
     -d '{"email":"admin@example.com","password":"ChangeThisPassword123!","newsletter_confirmed":false,"analytics_confirmed":false}'
   ```
...
To upgrade your Spraply installation to the latest version:
...
4. **Apply Database Migrations**
   ...
   The app container also runs migrations automatically on startup.
