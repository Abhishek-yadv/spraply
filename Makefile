...
# Show help
help:
	@echo "Spraply Monorepo Makefile"
	@echo ""
	@echo "Usage:"
	@echo "  ${COLOR_BLUE}make install${COLOR_RESET}              Install all dependencies (backend, frontend, pre-commit)"
	@echo "  ${COLOR_BLUE}make install-backend${COLOR_RESET}      Install FastAPI backend dependencies"
	...
	@echo "  ${COLOR_BLUE}make lint-backend${COLOR_RESET}         Run Ruff linter/formatter on FastAPI backend code"
...
install-backend:
	@echo "${COLOR_BLUE}Installing FastAPI backend dependencies...${COLOR_RESET}"
	@if ! command -v python3 &> /dev/null; then \
		echo "${COLOR_RED}Error: python3 is not installed.${COLOR_RESET}"; \
		exit 1; \
	fi
	@cd backend && python3 -m pip install -e . || { echo "${COLOR_RED}Failed to install backend dependencies.${COLOR_RESET}"; exit 1; }
	@echo "${COLOR_GREEN}FastAPI backend dependencies installed successfully!${COLOR_RESET}"
...
lint-backend:
	@echo "${COLOR_BLUE}Running Ruff linter and formatter on FastAPI backend code...${COLOR_RESET}"
	@if [ ! -d "backend" ]; then \
		echo "${COLOR_RED}Error: backend directory not found.${COLOR_RESET}"; \
		exit 1; \
	fi
	@if ! command -v ruff &> /dev/null; then \
		echo "${COLOR_YELLOW}ruff not found, installing...${COLOR_RESET}"; \
		python3 -m pip install ruff || { echo "${COLOR_RED}Failed to install ruff.${COLOR_RESET}"; exit 1; }; \
	fi
	@ruff check backend/app --fix || { echo "${COLOR_RED}Ruff linting failed.${COLOR_RESET}"; exit 1; }
	@ruff format backend/app || { echo "${COLOR_RED}Ruff formatting failed.${COLOR_RESET}"; exit 1; }
	@echo "${COLOR_GREEN}FastAPI backend code linted and formatted successfully!${COLOR_RESET}"
